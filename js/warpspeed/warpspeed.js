...
function WarpSpeed(targetId,config){
	...
	this.SPEED=config.speed==undefined||config.speed<0?1.8:config.speed;
	this.TARGET_SPEED=config.targetSpeed==undefined||config.targetSpeed<0?this.SPEED:config.targetSpeed;
	this.SPEED_ADJ_FACTOR=config.speedAdjFactor==undefined?0.5:config.speedAdjFactor<0?0:config.speedAdjFactor>1?1:config.speedAdjFactor;
	...
	this.BACKGROUND_COLOR=config.backgroundColor==undefined?"hsl(0, 0%, 85%)":config.backgroundColor;	
	var canvas=document.getElementById(this.targetId);
	canvas.width=1; canvas.height=1;
	var ctx=canvas.getContext("2d");
	ctx.fillStyle=this.BACKGROUND_COLOR;
	ctx.fillRect(0,0,1,1);
	ctx.fillStyle=config.starColor==undefined?"#131415":config.starColor;
	ctx.fillRect(0,0,1,1);
...
WarpSpeed.prototype={
	...
	draw:function(){
		...
		if(!this.PAUSED&&isVisible(canvas)){
			...
			this.size=(canvas.height<canvas.width?canvas.height:canvas.width)/(5/(this.STAR_SCALE<=0?0:this.STAR_SCALE));
			if(this.WARP_EFFECT) this.maxLineWidth=this.size/50;
			var ctx=canvas.getContext("2d");
			ctx.fillStyle=this.BACKGROUND_COLOR;
	...
	move:function(){
		var t=timeStamp(), speedMulF=(t-this.lastMoveTS)/(500/60);
		this.lastMoveTS=t;
